/*
 * CallCPP_Raytrace.cpp for sphereIntersect.
 *
 * @author Zhang Yifan
 */
...
// same as `Sphere::intersect`
static bool Sphere_intersect_Inst4(const Sphere& self, Ray& ray) {
  ...
  // Do the following quick check to see if there is even a chance
  // that an intersection here might be closer than a previous one
  if (instExpression(bFile, "Sphere_intersect", 2, 4, v - self.radius_ - ray.t_) > 0) {
    bFile << "node2@Sphere_intersect\n";
    return false;
  } else {
  ...
  // Test if the ray actually intersects the sphere
  float t = self.radSqr_ + v * v - dx * dx - dy * dy - dz * dz;
  if (instExpression(bFile, "Sphere_intersect", 5, 8, t) < 0) {
    bFile << "node6@Sphere_intersect\n";
    return false;
  } else {
  ...
  // ray direction and it is the closest so far
  t = v - (float) std::sqrt(t);
  if (instExpression(bFile, "Sphere_intersect", 10, 13, t - ray.t_) > 0
      || instExpression(bFile, "Sphere_intersect", 10, 14, t) < 0) {
...
(JNIEnv* env, jobject,
    ...
    jfloat index, jfloat x, jfloat y, jfloat z, jfloat rad, jfloat eyeX, jfloat eyeY, jfloat eyeZ, jfloat dirX, jfloat dirY, jfloat dirZ,
    jstring pathFile) {
