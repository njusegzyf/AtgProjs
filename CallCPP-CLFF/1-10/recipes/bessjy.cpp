...
void NR::bessjy(const DP x, const DP xnu, DP &rj, DP &ry, DP &rjp, DP &ryp)
{
	...
	if (x <= 0.0 || xnu < 0.0)
		printf("bad arguments in bessjy");
	...
	if (i >= MAXIT)
		printf("x too large in bessjy; try asymptotic expansion");
	rjl=isign*FPMIN;
	rjpl=h*rjl;
	rjl1=rjl;
	rjp1=rjpl;
	...
	if (x < XMIN) {
		...
		NR::beschb(xmu,gam1,gam2,gampl,gammi);
		...
		if (i > MAXIT)
			printf("bessy series failed to converge");
		rymu = -sum;
		ry1 = -sum1*xi2;
