...
public class Function {
     ...
     * 带参构造函数
     * @param name 函数名	
     * @param type 函数类型（返回值类型＋参数类型）
     * @param offset 在原文件中的偏移量
     * @param declarator 函数声明
     */
     ...
     * <p>删除每个函数调用出口节点的后继节点
     * <p>要么进入函数，要么跳过函数
     * <p>若函数末尾节点类型为return,则删除exit节点,为程序插桩做准备
     * @param function
     */
	...
	/**
     * 递归函数2，遍历函数的每个节点
     * <p>收集程序结束节点序列,注意到如果因条件语句中存在函数调用而对复合语句进行分离,此时结束节点可能会有所变动
     * @param node
     */
