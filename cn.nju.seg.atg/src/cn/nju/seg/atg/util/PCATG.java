...
public class PCATG extends ATG {
    ...
    if (!isCoveredTargetPath) {
      ...
      if (!isCoveredTargetPath) {
        ...
        while (parameterNum < max_num_of_predict_param && !isCoveredTargetPath) {
          ...
          /************************************************************************/
          // 从自变参数衍生池中衍生新的自变参数
          final List<Double> newParameterList = maxPath.getNewParameterList(paramIndex);
          // 更新自变参数衍生池
          final List<Double> newParameters = updateParameterList(newParameterList);
          /*************************************************************************/
  ...
  protected int generateTestDataForParam(int pathIndex, int paramIndex, String pathFile, double[] seed, int round, int nextRoundSeedIndex, int max_num_of_predict_param,
    ...
    // 判断是否已覆盖目标路径
    // @since 0.1
    boolean isCoveredTargetPath = getPathCoveredCondition(maxPath, paramIndex, pathFile, executedPathHandler);
    ...
    if (!isCoveredTargetPath) {
      ...
      TestBuilder.autoIncreasedParameterList.add(parameters[paramIndex]);
      // 判断是否已覆盖目标路径
      isCoveredTargetPath = getPathCoveredCondition(maxPath, paramIndex, pathFile, executedPathHandler);
